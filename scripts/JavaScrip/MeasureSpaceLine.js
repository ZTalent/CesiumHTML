class measureSpaceLine {
	...
	startCreate() {
			var $this = this;
			$this._slineArr = $this.calculateSpaceLine($this.viewer,$this.handler);
	}

	
	calculateSpaceLine(viewer, handler){
		// 取消双击事件-追踪该位置
	...
	destroy(){
		...
		if(this._slineArr)
		{
			for (var i = 0; i < this._slineArr.length; i++) {
		    var line = this._slineArr[i];
			 if(line)
			 {
